...
fn test_archive_editor_rename() {
    ...
    // Test rename
    {
        ...
        assert!(items.iter().any(|item| item.path.contains("renamed_file1")));
    }
...
fn test_archive_integrity() {
    ...
    // Test with extractor
    let extractor = BitExtractor::new(&lib, ExtractFormat::SevenZip);
    extractor.test(&archive_path).unwrap();
...
fn test_compress_with_aliases() {
    ...
    let normalized = items[0].path.replace('\\', "/");
    assert!(normalized.contains("backup/renamed.txt"));
